...
new_volume_t(int num_reads, int num_bases)
{
    ...
    safe_calloc(volume->metaData, uint8_t, num_bases);
...
clear_volume_t(volume_t* v)
{
    ...
    memset(v->metaData, 0, v->max_size);
...
add_one_seq(volume_t* volume, const char* s, const int size)
{
	...
	uint8_t* m = volume->metaData;
    int idx = volume->curr;
    int window_size=100;
    short window_gc_count=0;
    int gc_idx=volume->curr;
    int window_first_index=0;
	for (i = 0; i < size; ++i)
	{
	    if (window_gc_count>100)
            int y=0;
		uint8_t c = s[i];
		c = encode_table[c];
		...
		else if (window_first_index+window_size<size){
            m[gc_idx]=window_gc_count;
            gc_idx++;
            if(c==1 || c==2)
                window_gc_count++;
            if (s[window_first_index]=='C' || s[window_first_index]=='G')
                window_gc_count--;
            window_first_index++;
		}
		else{
...
dump_volume(const char* vol_name, volume_t* v)
{
	...
	SAFE_WRITE(v->metaData, uint8_t, v->curr, out);
	// 6) pac
...
load_volume(const char* vol_name)
{
	...
	// 5) metadata
	SAFE_READ(v->metaData, uint8_t, num_bases, in);
	// 6) pac
